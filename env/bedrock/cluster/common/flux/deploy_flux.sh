...
if ! mkdir $FLUX_MANIFESTS; then
    echo "ERROR: failed create direcotry $FLUX_MANIFESTS"
    exit 1
fi

echo "generating flux manifests with helm template"
if ! helm template . --name $RELEASE_NAME --namespace $KUBE_NAMESPACE --values values.yaml --set image.tag=1.0.1 --out-dir ./$FLUX_MANIFESTS --set git.url=$GITOPS_SSH_URL --set git.path=$GITOPS_PATH --set git.pollInterval=$GITOPS_POLL_INTERVAL -- set registry.acr.enabled=$ACR_ENABLED; then
