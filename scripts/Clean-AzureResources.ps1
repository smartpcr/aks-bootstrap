...
InitializeLogger -ScriptFolder $scriptFolder -ScriptName "Clean-AzureResources"
...
if ($bootstrapValues.global.components.aks -eq $true) {
    LogStep -Message "Delete AKS cluster '$($bootstrapValues.aks.clusterName)'..."
...
if ($bootstrapValues.global.components.dns -eq $true) {
    LogStep -Message "Delete DNA zone '$($bootstrapValues.dns.name)'..."
...
if ($bootstrapValues.global.components.appInsights -eq $true) {
    LogStep -Message "Delete app insights '$($bootstrapValues.appInsights.name)'..."
    az resource delete `
...
if ($bootstrapValues.global.components.cosmosdb.docDb -eq $true -and $IncludeData) {
    LogStep -Message "Delete docdb '$($bootstrapValues.cosmosdb.docDb.account)'..."
    az cosmosdb delete `
...
if ($bootstrapValues.global.components.cosmosdb.mongoDb -eq $true -and $IncludeData) {
    LogStep -Message "Delete mongodb '$($bootstrapValues.cosmosdb.mongoDb.account)'..."
    az cosmosdb delete `
...
if ($bootstrapValues.global.components.cosmosdb.graphDb -eq $true -and $IncludeData) {
    LogStep -Message "Delete graphDb '$($bootstrapValues.cosmosdb.graphDb.account)'..."
    az cosmosdb delete `
...
if ($bootstrapValues.global.components.redis -eq $true) {
    LogStep -Message "Delete redis cluster '$($bootstrapValues.redis.name)'..."
    az redis delete `
...
if ($bootstrapValues.global.components.acr -eq $true -and $IncludeData) {
    LogStep -Message "Delete ACR '$($bootstrapValues.acr.name)'..."
    az acr delete -g $bootstrapValues.acr.resourceGroup -n $bootstrapValues.acr.name --yes
}

if ($IncludeData) {
    LogStep -Message "Delete KV '$($bootstrapValues.kv.name)'..."
