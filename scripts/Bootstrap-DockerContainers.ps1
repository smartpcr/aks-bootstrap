...
function RemoveContainerByImageName {
    ...
    $existingContainer = $(docker ps -a -q --filter ancestor=$ImageName)
    if ($existingContainer) {
        Write-Host "`tRemoving existing container for image '$ImageName'..." -ForegroundColor Yellow
        docker container stop $existingContainer | Out-Null
        docker container rm $existingContainer | Out-Null
    }
}
...
InitializeLogger -ScriptFolder $scriptFolder -ScriptName "Bootstrap-DockerContainers"
...
if ($settings.global.bootstrap.installInfluxDb) {
    ...
    docker volume create --driver local --name influxdb --opt o=size=$maxDataSize --opt device=tmpfs --opt type=tmpfs

    docker run -d --name influxdb --net=$net -p "$($influxdbPort):8086" `
...
if ($settings.global.bootstrap.installTelegraf) {
    ...
    docker run -d --name telegraf --net=$net `
...
if ($settings.global.bootstrap.installPrometheus) {
    ...
    if ($installMethod -eq "choco") {
        ...
        RemoveContainerByImageName -ImageName $imageName
        docker run -d --name prometheus -p "$($prometheusPort):9090" -v "$($hostPrometheusFolder)/prometheus.yml:/etc/prometheus/prometheus.yml:ro" $imageName
    }
